  ...
  describe('editProfile', () => {
    ...
    it('should edit user email', async () => {
      ...
      const newVerification = {
        code: 'code',
        user: { verified: false, email: editProfileInput.input.email },
      };
      ...
      await service.editProfile(
        editProfileInput.userId,
        editProfileInput.input,
      );
      expect(usersRepository.findOne).toHaveBeenCalledTimes(1);
      expect(usersRepository.findOne).toHaveBeenCalledWith({
        where: { id: editProfileInput.userId },
      });
      expect(verificationsRepository.delete).toHaveBeenCalledWith({
        user: { id: editProfileInput.userId },
      });
      expect(verificationsRepository.create).toHaveBeenCalledTimes(1);
      expect(verificationsRepository.create).toHaveBeenCalledWith({
        user: {
  ...
  describe('verifyEmail', () => {
    ...
    it('should fail on exception', async () => {
      verificationsRepository.findOne!.mockRejectedValue(
        new Error('Could not find verification.'),
      );
      const result = await service.verifyEmail('code');
      expect(result).toMatchObject({
        ok: false,
        error: 'Could not find verification.',
      });
    });
